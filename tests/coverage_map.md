...
| Rust routed desync runtime | default `ciadpi` (Rust) owns the routed Linux fake/drop-sack suite, the environment-sensitive routed preflight, and an explicit `TCP_MD5SIG` capability probe for the `md5sig` subcase, so the explicit gate no longer shells out to `ciadpi-oracle` and only skips `md5sig` when the running kernel/runtime lacks that socket option | `make test-linux-routed-runtime`, `tests/test_linux_routed_runtime.py` |
...
| Rust CLI contract | default `ciadpi` (Rust) owns the `--help`, `--version`, invalid-argument, and invalid-value parse-failure surfaces without side-by-side hidden-C parity | `crates/ciadpi-bin/tests/cli.rs`, `make test` |
...
- Rust-owned defaults: `cargo test --workspace`, `make test`, `make cutover-gates`, the pull-request Linux CI job in `.github/workflows/ci.yml`, and `make test-windows-cross-check`. `make test-rust-oracle-diff` remains a focused subset target for the same committed-fixture cargo coverage that now also runs under `make test`.
