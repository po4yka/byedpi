...
| Rust binary integration | default `ciadpi` (Rust) owns a Rust-native live binary lane for SOCKS4, SOCKS5, HTTP CONNECT success/failure, SOCKS5 UDP associate, UDP fake bursts, upstream SOCKS chaining, connection churn, IPv6 where loopback is available, `--max-conn`, Shadowsocks env startup, `--no-domain`, `--no-udp`, and failed upstream connect handling | `crates/ciadpi-bin/tests/runtime_integration.rs`, `make test` |
