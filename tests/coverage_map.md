...
| Rust routed desync runtime | default `ciadpi` (Rust) owns the routed Linux fake/md5sig/drop-sack suite and its environment-sensitive preflight, so the explicit gate no longer shells out to `ciadpi-oracle` before deciding whether routed fake-path cases should skip on the current machine | `make test-linux-routed-runtime`, `tests/test_linux_routed_runtime.py` |
