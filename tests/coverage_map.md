...
| Rust binary integration | default `ciadpi` (Rust) owns a Rust-native live binary lane for SOCKS5 echo, HTTP CONNECT echo, `--no-domain` rejection, and failed upstream connect handling | `crates/ciadpi-bin/tests/runtime_integration.rs`, `make test` |
